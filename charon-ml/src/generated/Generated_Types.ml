...
(** Const Generic Values. Either a primitive value, or a variable corresponding to a primitve value *) type  const_generic = 
...
(** The id of a translated item. *) and  item_id = 
 ...
 *) type 'a0 binder = { binder_params :  generic_params  ;binder_value : 'a0  (** Named this way to highlight accesses to the inner value that might be handling parameters
 ...
 | BKTraitType of  trait_decl_id * trait_item_name  (** The parameters of a generic associated type. *)
 ...
 *) and  builtin_impl_data = 
 ...
 | BuiltinNoopDestruct (** An impl of [Destruct] for a type with no drop glue. *)

 | BuiltinUntrackedDestruct (** An impl of [Destruct] for a type parameter, which we could not resolve because
...
(** A const generic variable in a signature or binder. *) and  const_generic_param = { index :  const_generic_var_id  (** Index identifying the variable among other variables bound at the same level. *);name :  string  (** Const generic name *);ty :  literal_type  (** Type of the const generic *) }
 ...
 *) and  generic_params = { regions : ( region_param) list  ;types : ( type_param) list  ;const_generics : ( const_generic_param) list  ;trait_clauses : ( trait_param) list  ;regions_outlive : (((( region),( region)) outlives_pred) region_binder) list  (** The first region in the pair outlives the second region *);types_outlive : (((( ty),( region)) outlives_pred) region_binder) list  (** The type outlives the region *);trait_type_constraints : (( trait_type_constraint) region_binder) list  (** Constraints over trait associated types *) }
 ...
 *) and 'a0 region_binder = { binder_regions : ( region_param) list  ;binder_value : 'a0  (** Named this way to highlight accesses to the inner value that might be handling parameters
        incorrectly. Prefer using helper methods.
     *) }

 and  region_id = RegionId.id [@visitors.opaque]

(** A region variable in a signature or binder. *) and  region_param = (region_id, string option) indexed_var

(** The value of a trait associated type. *) and  trait_assoc_ty_impl = { value :  ty   }
 ...
 *) and  trait_param = { clause_id :  trait_clause_id  (** Index identifying the clause among other clauses bound at the same level. *);span : ( span) option  ;trait : ( trait_decl_ref) region_binder  (** The trait that is implemented. *) }
 ...
 *) and  trait_ref = ( trait_ref_contents) hash_consed 
 ...
 and  trait_ref_contents = { kind :  trait_ref_kind  ;trait_decl_ref : ( trait_decl_ref) region_binder  (** Not necessary, but useful *) }
 ...
 *) and  trait_ref_kind = 
 ...
 | ItemClause of ( trait_ref) * trait_item_name * trait_clause_id  (** A clause defined on an associated type. This variant is only used during translation; after
 ...
 | BuiltinOrAuto of  builtin_impl_data *( trait_ref) list *(( trait_item_name)*( trait_assoc_ty_impl)) list  (** A trait implementation that is computed by the compiler, such as for built-in trait
 ...
 *) and  trait_type_constraint = { trait_ref :  trait_ref  ;type_name :  trait_item_name  ;ty :  ty   }
 ...
 | TRef of  region * ty * ref_kind *(( ty_view_field) list) option  (** A borrow *)
 ...
 | TPtrMetadata of  ty  (** As a marker of taking out metadata from a given type
          The internal type is assumed to be a type variable
       *)

 | TError of  string  (** A type that could not be computed or was incorrect. *)

 and  ty_view_field = { path : ( string) list  ;mutbl :  ref_kind   }
...
(** A type variable in a signature or binder. *) and  type_param = (type_var_id, string) indexed_var
[@@deriving show, eq, ord, 
                                    visitors {
                                        name = "iter_ty";
                                        monomorphic = ["env"];
 ...
 *) and  item_source = 
 ...
 | VTableMethodShimItem (** The method shim wraps a concrete implementation of a method into a function that takes [dyn
 ...
 *) and  layout = { size : int option  (** The size of the type in bytes. *);align : int option  (** The alignment, in bytes. *);discriminant_layout : ( discriminant_layout) option  (** The discriminant's layout, if any. Only relevant for types with multiple variants. *);uninhabited : bool  (** Whether the type is uninhabited, i.e. has any valid value at all.
...
(** See the comments for [Name] *) and  path_elem = 
 ...
 | PeInstantiated of (( generic_args) binder)  (** This item was obtained by instantiating its parent with the given args. The binder binds
 ...
 *) and  ptr_metadata = 
 ...
 | Length (** Metadata for [[T]] and [str], and user-defined types
 ...
 | VTable of  type_decl_ref  (** Metadata for [dyn Trait], referring to the vtable struct,
          also for user-defined types that directly or indirectly contain a [dyn Trait].
          Of type [&'static vtable]
       *)

 | InheritFrom of  ty  (** Unknown due to generics, but will inherit from the given type.
 ...
 *) and  repr_algorithm = 
 ...
 | Rust (** The default layout algorithm. Used without an explicit [ŗepr] or for [repr(Rust)]. *)
 ...
 *) and  repr_options = { repr_algo :  repr_algorithm  ;align_modif : ( alignment_modifier) option  ;transparent : bool  ;explicit_discr_type : bool   }
...
(** A type declaration.
    ...
    inlined in MIR.
 *) and  type_decl = { def_id :  type_decl_id  ;item_meta :  item_meta  (** Meta information associated with the item. *);generics :  generic_params  ;src :  item_source  (** The context of the type: distinguishes top-level items from closure-related items. *);kind :  type_decl_kind  (** The type kind: enum, struct, or opaque. *);layout : ( layout) option  (** The layout of the type. Information may be partial because of generics or dynamically-
        sized types. If rustc cannot compute a layout, it is [None].
     *);ptr_metadata :  ptr_metadata  (** The metadata associated with a pointer to the type. *);repr : ( repr_options) option  (** The representation options of this type declaration as annotated by the user.
 ...
 and  variant = { span :  span  ;attr_info :  attr_info  ;variant_name :  string  ;fields : ( field) list  ;discriminant :  literal  (** The discriminant value outputted by [std::mem::discriminant] for this variant.
