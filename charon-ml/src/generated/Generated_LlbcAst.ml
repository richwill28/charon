...
module StatementId = IdGen ()
 ...
 and  statement = { span :  span  ;statement_id :  statement_id  (** Integer uniquely identifying this statement among the statmeents in the current body. To
     ...
     *);kind :  statement_kind  ;comments_before : ( string) list  (** Comments that precede this statement. *) }
...
(** A raw statement: a statement without meta data. *) and  statement_kind = 
 ...
 | SwitchInt of  operand * literal_type *((( literal) list)*( block)) list * block  (** Gives the integer type, a map linking values to switch branches, and the
